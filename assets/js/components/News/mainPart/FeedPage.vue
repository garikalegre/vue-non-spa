<template>
    <v-card color="white" class="mt-5 news-common-card">
        <v-layout row wrap>
            <v-flex d-flex justify-center xs1 news-item-avatar-box>
                <v-avatar @click="gotoProfile(feed.user)">
                    <img :src="avatarUrl(feed.user)"/>
                </v-avatar>
            </v-flex>
            <v-flex xs11>
                <div class="text-box">
                    <div class="title-box">
                        <h6 @click="titleRedirect(feed)" class="title-names mb-2">{{feed.user.username}}
                            {{feed.content.title ? feed.content.title : ''}}
                            <span class="title-date">{{parseDate(feed.created.date)}}</span>
                        </h6>
                        <span class="type-label" :style="{background: typeLabel}">{{renderType(feed.type)}}</span>
                    </div>

                    <p class="mt-2 description">{{feed.content.content}}</p>
                    <img v-if="feed && feed.type === 'Post' && feed.content && feed.content.giphy" :src="feed.content.giphy" class="max-width-img">
                    <p v-if="feed.content.location" class="mt-2 news-item-location">
                        {{feed.content.location}}</p>
                    <p class="mt-2 news-item-sport">{{parseSport(feed.user.sports_full)}}</p>
                    <img v-if="feed.content.images.length" v-for="item in feed.content.images" :src="item"
        ...
        <news-comment
                ...
                v-for="(item, index) in feed.comment"
                :key="index"
                :comment="item"
                :parseDate="parseDate"
...
<script>
    ...
    export default {
        ...
        methods: {
            ...
            likeHandler() {
                this.$http.post(`/feed/${this.feed.id}/like/store`).then(response => {
                    this.liked = true;
                    this.likedNumber += 1;
                });
            },
            unlikeHandler() {
                this.$http.post(`/feed/${this.feed.id}/like/remove`).then(response => {
                    this.liked = false;
                    this.likedNumber -= 1;
                });
            },
            ...
            titleRedirect(feed){
                if (feed.type == 'Event') {
                    window.location.href = `/event/${feed.content.slug}`;
                } else {
                    this.gotoProfile(feed.user);
                }
            }
        },
        created() {
            this.liked = this.feed.liked;
            this.likedNumber = this.feed.like.length;
            this.commentsNumber = this.feed.comment.length;
        }
    }
</script>
